...
server <- function(input, output, session) {
 ...
 shiny::observeEvent(input$azmetStationPast24Hours, { # Keep selected station between tabs
  ...
  latestConditionsData <- 
   shiny::eventReactive(lw15min(), {
  ...
  past24HoursCardGraphs <- 
    shiny::eventReactive(c(input$azmetStationPast24Hours, lw15min()), {
  ...
  past24HoursCardLayout <- 
    shiny::eventReactive(c(input$azmetStationPast24Hours, lw15min(), past24HoursCardGraphs()), {
  ...
  past30DaysCardGraphs <- 
    shiny::eventReactive(c(input$azmetStationPast30Days, lwdaily()), {
  ...
  past30DaysCardLayout <- 
    shiny::eventReactive(c(input$azmetStationPast30Days, lwdaily(), past30DaysCardGraphs()), {
  ...
  output$latestConditionsTableFooter <- 
    shiny::renderUI({
      shiny::req(latestConditionsData())
      fxn_latestConditionsTableFooter()
    })
  
  output$latestConditionsTitle <- 
    shiny::renderUI({
      shiny::req(latestConditionsData())
  ...
  output$past24HoursCardLayoutFooter <- 
    shiny::renderUI({
      shiny::req(lw15min())
      fxn_past24HoursCardLayoutFooter()
    })
  
  output$past24HoursLatestDataUpdate <- 
    shiny::renderUI({
      # shiny::req(lw15min())
  ...
  output$past24HoursTitle <- 
    shiny::renderUI({
      shiny::req(lw15min())
      fxn_past24HoursTitle()
    })
  
  output$past30DaysCardLayout <- 
    shiny::renderUI({
      bslib::layout_column_wrap(
        !!!past30DaysCardLayout(),
        #class = ,
  ...
  output$past30DaysCardLayoutFooter <- 
    shiny::renderUI({
      shiny::req(lwdaily())
      fxn_past30DaysCardLayoutFooter()
    })
  
  output$past30DaysLatestDataUpdate <- 
    shiny::renderUI({
      # shiny::req(lwdaily())
